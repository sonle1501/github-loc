...
public class LocProcessor {
    ...
    public record FileInfo(
            int loc, Set<String> languageSet, int comments, int blanks, Map<String, Integer> locByLang) {}

    public FileInfo processFileInfo(Path filePath, FileCounter counter) {
    ...
    private Map<Language, Counts> processFileCountResult(FileCounter counter, Path filePath) {
        try {
            ...
            throw new GithubLocException(ErrorCode.FILE_PROCESSING_ERROR, "Failed to count LOC for file: " + filePath, e);
        }
    }
